...
class NetworkCnn(tnn.Module):
    ...
    def __init__(self):
        ...
        self.global_pool = tnn.AdaptiveMaxPool1d(1)
        self.l = tnn.Linear(50, 1)
    ...
    def forward(self, input, length):
        ...
        x = tnn.functional.relu(self.c1(x))
        x = self.max_pool(x)
        x = tnn.functional.relu(self.c2(x))
        x = self.max_pool(x)
        x = tnn.functional.relu(self.c3(x))
