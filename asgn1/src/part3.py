...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.shape[0], -1)  # flattening input
        x = F.relu(self.f1(x))
        x = F.relu(self.f2(x))
        x = F.relu(self.f3(x))
        x = F.log_softmax(x, dim=1)  # preserve batch dimension
        return x
class CNN(nn.Module):
    """
    ...
    def __init__(self):
        super().__init__()
        self.convol1 = nn.Conv2d(1, 10, 5, 1)
        self.convol2 = nn.Conv2d(10, 50, 5, 1)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.convol1(x))
        x = self.maxpool(x)
        x = F.relu(self.convol2(x))
        x = self.maxpool(x)
...
class NNModel:
    ...
    def view_batch(self):
        ...
        for images, labels in self.trainloader:
            ...
            _labels = labels
            break
        _labels = _labels.reshape(8,8)
        _images = _images.reshape(8,8,28,28).permute(0,2,1,3).reshape(224,224)
